    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      {isVisible && (
        <div className="flex flex-col gap-1">
          ...
          <p className="text-gray-400 text-base ml-4">Acompanhe o desempenho da sua operação hoje.</p>
        </div>
      )}
          ...
          <div className="bg-[#0d0d10] border border-white/5 rounded-3xl overflow-hidden shadow-2xl">
            <div className="p-6 border-b border-white/5 flex items-center justify-between bg-white/[0.01]">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xs font-bold text-gray-400 uppercase tracking-[2px]">
                  Últimas vendas do mês
                </h2>
              </div>
              <button
                onClick={() => setShowCalendar(!showCalendar)}
                className="px-5 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-full text-xs font-bold uppercase tracking-wider transition-all"
              >
            ...
            <div className="p-6">
              {showCalendar ? (
                <div className="bg-[#0a0a0c] p-4 rounded-2xl border border-white/5">
                  {vendasLoading ? <div className="p-10 text-center text-gray-500 text-lg">Sincronizando...</div> : <MyCalendar data={vendasData!} />}
                </div>
              ) : (
                <div className="space-y-4">
                  {vendasLoading ? (
                    <div className="py-20 text-center animate-pulse text-gray-600 text-lg">Carregando dados...</div>
                  ) : vendasData?.GetVendaByUsuarioID?.length ? (
                    vendasData.GetVendaByUsuarioID.map((venda: any) => (
                      <div
                        key={venda.id}
                        className="group flex items-center justify-between p-5 bg-white/[0.02] hover:bg-white/[0.05] border border-white/5 rounded-2xl transition-all"
                      >
                        <div className="flex items-center gap-5">
                          ...
                          <div>
                            <p className="text-base font-bold text-white">{DateToVim(venda.data_venda)}</p>
                    ...
                    <div className="py-12 text-center text-gray-500 italic text-base">Sem registros neste período.</div>
                  )}
                </div>
              )}
            </div>
        ...
        <div className="lg:col-span-5">
          <div className="bg-[#0d0d10] border border-white/5 rounded-3xl h-full shadow-2xl overflow-hidden flex flex-col">
            ...
            <div className="p-6 flex-1 custom-scrollbar overflow-y-auto max-h-[600px]">
              {metasLoading ? (
                <div className="p-10 text-center text-gray-600 text-lg">Calculando metas...</div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/85 backdrop-blur-md">
          <div className="bg-[#0d0d10] border border-white/10 w-full max-w-lg rounded-3xl p-8 shadow-2xl animate-in zoom-in-95 duration-300">
            <div className="flex justify-between items-start mb-8">
              <div>
                <h3 className="text-2xl font-bold text-white">Detalhes da Venda</h3>
                <p className="text-emerald-500 text-sm font-bold uppercase tracking-widest mt-2">
                  {DateToVim(selectedVenda.data_venda)}
                </p>
              </div>
              <button onClick={() => setSelectedVenda(null)} className="text-gray-500 hover:text-white text-xl">✕</button>
            </div>
              ...
              <div className="p-5 bg-white/5 rounded-2xl border border-white/5">
                ...
                <ul className="space-y-4">
                  {selectedVenda.venda_detalhe.map((d: any) => (
                    <li key={d.id} className="flex justify-between items-center text-white text-base">
                      <span>{d.produto.nome} <span className="text-gray-500 ml-2 text-sm font-medium">x{d.quantidade}</span></span>
                      <span className="font-mono text-emerald-400 font-bold">+{d.pontos} pts</span>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <button
              ...
              className="w-full mt-8 py-5 bg-white/5 hover:bg-white/10 text-white rounded-2xl font-bold uppercase tracking-[2px] text-xs transition-all border border-white/5"
            >
              Fechar Detalhes
            </button>
          </div>
